...
subtitle: "statistical data assimilation"
...
But it has spread out to other geoscience disciplines and beyond.
    For instance, to name a few domains, it has been applied in
    greenhouse gas inverse modeling, air quality forecasting,
    extra-terrestrial atmosphere forecasting , detection and attribution
    in climate sciences, geomagnetism re-analysis , and ice-sheet
    parameter estimation and forecasting. It has also been used in
    petroleum reservoir estimation, in adaptive optics for extra large
    telescopes, and highway traffic estimation.
...
Since it is based on [Gaussian]{style="color: magenta"} statistics
    ...
    the more general *particle filter*seeAdvanced Course. Nonetheless,
    it turns out to be an excellent
    [approximate]{style="color: magenta"} algorithm for the filtering
    problem.
...
-   However, as we shall see in the Advanced Course Lectures, in
...
In [EnKF]{style="color: magenta"}s, the initial system state and its
...
-   A [new forecast ]{style="color: magenta"}is issued from the
        analyszd state and this procedure is repeated until the end of
...
Time-dependent variational methods
    ...
    window to find the trajectory that minimises the misfit ($J_{0}$)
...
-   Fundamentally, one cannot hope to represent the full error
        ...
        a f[ew modes]{style="color: magenta"} $m\ll n$, usually from a
...
modeling will be dealt with in the [Advanced
Course]{style="color: magenta"}, where we will consider:
...
In the [Advanced]{style="color: magenta"} Course, we will study in
    more detail:
