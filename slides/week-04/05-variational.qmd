...
Variational DA is based on an [adjoint
    approach]{style="color: magenta"} that is explained in the previous
    Lecture.
...
It also assumes that we know and that we can [invert]{style="color: magenta"} the matrices $\mathbf{R}$ and $\mathbf{B}$.
...
-   $\boldsymbol{\epsilon}^{\mathrm{o}}_{k}$ is the [observation error ]{style="color: magenta"} with
...
Now we add the [dynamics]{style="color: magenta"} described by the [state equation]{style="color: magenta"},
...
Now we take the null products (according to the tangent state
    ...
    \delta J & =\left(\mathbf{x}_{0}-\mathbf{x}^{\mathrm{b}}_{0}\right)^{\mathrm{T}}\left(\mathbf{P}^{\mathrm{b}}_{0}\right)^{-1}\delta \mathbf{x}_{0}\\
     & +\sum_{k=0}^{K}\left(\mathbf{H}_{k}(\mathbf{x}_{k})-\mathbf{y}_{k}\right)^{\mathrm{T}}\mathbf{R}_{k}^{-1}\mathrm{H}_{k}\delta\mathbf{x}_{k}\\
