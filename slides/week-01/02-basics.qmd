...
-   Newer approaches (see [Advanced]{style="color: magenta"} Course): nudging methods, reduced methods, ensemble methods and hybrid methods that combine variational and statistical approaches, [Machine/Deep Learning based approaches]{style="color: magenta"}.
...
**Forward problem:** [Given $\theta,$]{style="color: blue"} [$z_{0},$]{style="color: blue"}[find $z(t)$]{style="color: orange"} for $t\ge t_{0}.$

**Inverse problem:** [Given $z(t)$]{style="color: orange"}for $t\ge t_{0},$ [find $\theta\in\Theta.$]{style="color: blue"}

## Observations {.smaller}

Observation [equation]{style="color: magenta"}: $$f(t,\theta)=\mathcal{H}z(t,\theta),$$ where $\mathcal{H}$ is the observation operator --- to account for the fact that observations are never completely known (in space-time).

Usually we have a finite number of discrete (space-time) [observations]{style="color: magenta"} $$\left\{\widetilde{y}_{j}\right\} _{j=1}^{n},$$ where $$\widetilde{y}_{j}\approx f(t_{j},\theta).$$

## Noise-free and noise data {.smaller}

[Noise-free:]{style="color: magenta"} $$\widetilde{y}_{j}=f(t_{j},\theta)$$

[Noisy Data:]{style="color: magenta"} $$\widetilde{y}_{j}=f(t_{j},\theta)+\varepsilon_{j},$$ where $\varepsilon_{j}$ is error and requires that we introduce [variability/uncertainty]{style="color: magenta"} into the modeling and analysis.
...
The first class will be treated by linear algebra and [optimization
    ]{style="color: magenta"}methods.

The latter can be treated by a [Bayesian (filtering)
    ]{style="color: magenta"}approach, and by weighted least-squares,
...
Finally, since most inverse problems cannot be solved explicitly,
    ...
    for their solution see @asch2022toolbox
...
The [error covariance matrix]{style="color: blue"} associated with
    ...
    ([\[eq:state\]](#eq:state){reference-type="ref"
    reference="eq:state"}) by random or systematic errors.
...
-   $H$ is an [observation operator]{style="color: magenta"}
