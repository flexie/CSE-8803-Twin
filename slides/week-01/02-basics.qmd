...
See also our recent publication [An uncertainty-aware digital shadow for underground multimodal CO2 storage monitoring](https://academic.oup.com/gji/article/242/1/ggaf176/8142541) [@gahlot2025uncertainty]
...
**Idea:** is to replace the ill-posed problem for
    ...
    where $\beta$ is "suitably chosen" regularization/penalization parameter, see below for details.


- When it is done correctly, TR provides convexity and compactness[^10].
...
[^10]: *Convexity* means that the function resembles a quadratic function, $f(x) = x^2$ with positive second derivative; *compactness* means that any infinite sequence of functions must get arbitrarily close to some function of the space.
...
We have here, similarly to the [bias-variance
    trade-off]{style="color: magenta"} of Chapter 5 (and 9), a competition
...
**[Cross-validation]{style="color: magenta"}**, as we explained in Chapter 9, is a way of using the observations themselves to  estimate a parameter.
...
::: center
**[DATA ASSIMILATION]{style="color: blue"}**[^11]
:::
