...
$$A[\mathbf{m}]\mathbf{x}=\mathbf{q}$${#eq-A}

where $A[\mathbf{m}]$ represents the discretized PDE, $\mathcal{m}$ spatially varying coefficients of the PDE, and $\mathbf{q}$ a source term. 
...
- PyTorch, TensorFlow (Python), or Flux.jl (Julia)

::: {.callout-caution}
While convenient, AD as part of ML tools (known as backpropagation)

- does not scale
- integrate poorly with existing CSE codes
:::
