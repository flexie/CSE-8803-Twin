...
The Kalman filter was invented[^1] in 1960 by R. E. Kálmán to solve this sort of problem in a mathematically optimal way.
...
-   Then the theory of [Itô]{style="color: magenta"} provides a detailed solution of the problem of optimal filtering as well as rigorous existence and uniqueness results... see [@law2015data; @sarkka2023bayesian].
...
Since $\mathbf{y}_{i}\equiv\mathbf{y}$ was assumed, the normalized
    ...
    $$\mathbf{X}_{i}^{{\rm a}}=\mathbf{X}_{i}^{{\rm f}}+\mathbf{K}\left(\mathbf{0}-\mathbf{H}\mathbf{X}_{i}^{{\rm f}}\right)=\left(\mathbf{I}_{n}-\mathbf{K}\mathbf{H}\right)\mathbf{X}_{i}^{{\rm f}},$${#eq-anomaly-update}

<!-- \label{eq:anomaly-update} -->
where
...
## Avoidance of Adjoints for Observations{.smaller}

\begin{aligned}
&\begin{aligned}
\overline{\mathbf{y}}^{\mathrm{f}} & =\frac{1}{m} \sum_{i=1}^m \mathcal{H}\left(\mathbf{x}_i^{\mathrm{f}}\right), \\
\mathbf{P}^{\mathrm{f}} \mathbf{H}^{\mathrm{T}} & =\frac{1}{m-1} \sum_{i=1}^m\left(\mathbf{x}_i^{\mathrm{f}}-\overline{\mathbf{x}}^{\mathrm{f}}\right)\left[\mathbf{H}\left(\mathbf{x}_i^{\mathrm{f}}-\overline{\mathbf{x}}^{\mathrm{f}}\right)\right]^{\mathrm{T}} \\
& \simeq \frac{1}{m-1} \sum_{i=1}^m\left(\mathbf{x}_i^{\mathrm{f}}-\overline{\mathbf{x}}^{\mathrm{f}}\right)\left[\mathcal{H}\left(\mathbf{x}_i^{\mathrm{f}}\right)-\overline{\mathbf{y}}^{\mathrm{f}}\right]^{\mathrm{T}}, \\
\mathbf{H} \mathbf{P}^{\mathrm{f}} \mathbf{H}^{\mathrm{T}} & =\frac{1}{m-1}\left[\mathbf{H}\left(\mathbf{x}_i^{\mathrm{f}}-\overline{\mathbf{x}}^{\mathrm{f}}\right)\right]\left[\mathbf{H}\left(\mathbf{x}_i^{\mathrm{f}}-\overline{\mathbf{x}}^{\mathrm{f}}\right)\right]^{\mathrm{T}} \\
& \simeq \frac{1}{m-1} \sum_{i=1}^m\left[\mathcal{H}\left(\mathbf{x}_i^{\mathrm{f}}\right)-\overline{\mathbf{y}}^{\mathrm{f}}\right]\left[\mathcal{H}\left(\mathbf{x}_i^{\mathrm{f}}\right)-\overline{\mathbf{y}}^{\mathrm{f}}\right]^{\mathrm{T}} .
\end{aligned}\\
\end{aligned}
The key assumption in these derivations is the approximation 

$$\mathbf{H}\left(\mathbf{x}_i^{\mathrm{f}}-\overline{\mathbf{x}}^{\mathrm{f}}\right) \simeq \mathcal{H}\left(\mathbf{x}_i^{\mathrm{f}}\right)-\overline{\mathbf{y}}^{\mathrm{f}}$$
...
$$\left[\mathbf{Y}_{\mathrm{f}}\right]_{i,k}=\frac{\mathcal{H}_{k}(\mathbf{x}_{i,k}^{\mathrm{f}})-\overline{\mathbf{y}}^{\mathrm{f}}-\mathbf{u}_i-\overline{\mathbf{u}}}{\sqrt{m-1}}$$
