...
-   the [conditional probability,
        ...
        represents the stochastic [dynamics model]{style="color: magenta"}, and can be a probability density
...
Using the nonlinear filtering model [-@eq-filter] and the Markov property, we can express
...
What we need is a [fixed number of computations per time-step]{style="color: magenta"}.
...
:::{#def-BayesFilter} 
*[Bayesian filtering]{style="color: magenta"} is the recursive computation of the
marginal posterior distribution, $$p(\mathbf{x}_{k}\mid\mathbf{y}_{1:k})$$ known as the filtering
...
for the [Kalman filter]{style="color: magenta"}, already derived. We recall the linear, Gaussian state-space model,
$$\begin{aligned}
...
The EKF is then based on [Gaussian approximations]{style="color: magenta"} of the filtering densities,
...
:::{#fig-EKF_pendulum}
![](../images/fig-EKF_pendulum){width=70%}
...
In practice though, the particle filter cannot yet be applied to
    ...
    curse of dimensionality"). Though recent work by [@friedemann2023framework] has improved this by sophisticated parallel computing.
